...
- Version `0.1.2` certifies official DSH `0.1.2-rc.1` at commit `a66e4702047846cdaa10c66c9d3df3951f5ea70d`; run `npm test` with `DSH_CORE_PATH` set to that exact source checkout before changing the claim.
- Source-seam certification must assert the root and `@deepseek-ai/dsh-mcp-client` versions plus every stdio config and lifecycle marker consumed by `cordis.patch.yml`. Windows CI owns runtime applicability; Linux may certify only the platform-neutral source seam.
