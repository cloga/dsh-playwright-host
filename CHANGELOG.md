...
- Add exact compatibility certification for DSH Core `0.1.3-alpha.1` at commit `d347e703908d0406b7a7ef80e3a0e594d86b2215` while retaining the `0.1.2-rc.1` certification.
- Verify the alpha.1 `@deepseek-ai/dsh-mcp-client` config and lifecycle markers consumed by `cordis.patch.yml`, including stdio command/args/env/cwd forwarding, startup readiness and failure behavior, reconnect supervision, effect-owned disposal, tool replacement, and per-call timeout.
