...
- Add real upstream behavior gates for initialization, strict startup failure, optional Host-route isolation after pagination failure, reconnect recovery, resource/provider and PTC cleanup, probe/child-process quiescence, resource URI templates, and native/PTC resource projection. Build the exact upstream Host library outputs before loader-based tests so `typert-loader` validates the intended MCP failure in isolation.
...
- Preserve Microsoft Playwright MCP `@playwright/mcp@0.0.80`, isolated installed Edge, Host-wide shared-process boundaries, and the immutable merge-gated Release workflow. cloga/deepseek-harness#33 has merged with green CI; this version remains blocked because the current `0.1.5` Desktop prerelease reports `immutable:false`, its publish job failed, and the remote managed Check was skipped. Windows Ops must repin the corrected release and cloga/dsh-windows-ops#161 must explicitly clear this bundle before merge/release.
